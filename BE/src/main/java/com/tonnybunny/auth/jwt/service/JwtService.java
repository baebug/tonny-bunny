package com.tonnybunny.auth.jwt.service;


import com.auth0.jwt.JWT;
import com.auth0.jwt.algorithms.Algorithm;
import com.auth0.jwt.interfaces.DecodedJWT;
import com.tonnybunny.auth.jwt.JwtProperties;
import com.tonnybunny.auth.jwt.JwtToken;
...
public class JwtService {
	...
	public JwtToken joinJwtToken(String email) {
		...
		if (userRefreshToken == null) {
			...
			//access, refresh 토큰 생성
			JwtToken jwtToken = jwtProviderService.createJwtToken(user.getSeq(), user.getEmail());
		...
		//refresh 토큰이 있는 사용자(기존 사용자)
			...
			if (accessToken != null) {
				...
				JwtToken newJwtToken = jwtProviderService.createJwtToken(user.getSeq(),
					user.getEmail());
	...
	public String validAccessToken(String accessToken) {
		...
		try {
			DecodedJWT verify = JWT.require(Algorithm.HMAC512(JwtProperties.SECRET)).build()
				.verify(accessToken);
	 ...
	 */
	@Transactional
	public JwtToken validRefreshToken(String email, String refreshToken) {
		...
		//refresh 토큰이 만료됨 -> access, refresh 토큰 모두 재발급
			...
			JwtToken newJwtToken = jwtProviderService.createJwtToken(findUser.getSeq(),
				findUser.getEmail());
	...
	public RefreshToken sameCheckRefreshToken(UserEntity findUser, String refreshToken) {
